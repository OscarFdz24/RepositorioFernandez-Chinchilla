...
<head>
    ...
    <title>FitSyncronizer | Listar Perfiles</title>
    ...
    <link rel="stylesheet" href="web/css/listarPerfiles.css">
...
<body>
    ...
    <main>
        ...
        <section class="full-view-content">
            ...
            <div id="content-container" class="mt-5 mb-5">
                <div class="row">
                    <h1 class="mb-5">¡Explora todos los perfiles de tu alrededor!</h1>
                    <?php if (count($perfiles) > 0) : ?>
                        <?php foreach ($perfiles as $perfil) : ?>
                            <div class="col-4">
                                <div class="card">
                                    <div class="perfil">
                                        <img src="web/fotosPerfiles/<?= htmlspecialchars_decode($perfil->getImagen()) ?>" alt="foto de perfil" class="perfil-img img-fluid">
                                        <h3><?= htmlspecialchars_decode($perfil->getNombreUsuarioProfesional()) ?></h3>
                                        <div class="buttons buttons--modifier">
                                            <a href="index.php?accion=mostrarPerfil&id=<?= $perfil->getId() ?>"> Mostrar perfil</a><br>

                                            <?php
                                            if (Sesion::existeSesion()) {
                                                $idUsuario = Sesion::getUsuario()->getId();
                                                $idPerfil = $perfil->getId();
                                                $numLikes=$likeDAO->countByIdPerfil($perfil->getId());
                                                // Verificar si $likeDAO está definido
                                                if (isset($likeDAO)) {
                                                    $existeLike = $likeDAO->existByIdUsuarioIdPerfil($idUsuario, $idPerfil);
                                                } else {
                                                    echo "Error: \$likeDAO no está definido.<br>"; // Depuración
                                                }
                                            ?>
                                                <?php if (isset($existeLike) && $existeLike) : ?>
                                                    <i class="fa-solid fa-thumbs-up iconoLikeOn" data-idPerfil="<?= $perfil->getId() ?>"></i>
                                                <?php else : ?>
                                                    <i class="fa-regular fa-thumbs-up iconoLikeOff" data-idPerfil="<?= $perfil->getId() ?>"></i>
                                                <?php endif; ?>
                                            <?php } ?>
                                            <div class="numLikes numLikes-<?= $perfil->getId() ?>"><?= $numLikes ?></div>
                                        </div>
                        ...
                        <?php endforeach; ?>
                    <?php else : ?>
                        <p>No hay ningun perfil actualmente</p>
                    <?php endif; ?>
                </div>
            </div>
            </div>
    ...
    <script src="web/archivosJS/likes.js"></script>
</body>
